...
const flowSteps = [
  ...
  },
  {
    ...
    desc: 'Сервис загрузки извлекает текст из файла и считает уникальный отпечаток. Для PDF-сканов без текстового слоя текст автоматически восстанавливается через OCR (PaddleOCR-VL). Если такой файл уже загружали — возвращается ссылка на существующий документ. Если новый — файл сохраняется на диск, и сообщение об этом ставится в очередь.',
  },
  {
    ...
    <section id="classifier" class="ref-section">
      ...
      <div class="ref-block padded mb-3">
        ...
        <ol class="classifier-steps">
          <li>Из файла извлекается текст. Для PDF без текстового слоя (сканы, фото-копии) система автоматически переходит к OCR: страницы рендерятся в изображения и отправляются в <strong>PaddleOCR-VL 1.5</strong>, которая возвращает текст вместе с разметкой таблиц и порядком чтения. Текстовый путь занимает миллисекунды, OCR — около 5–9 секунд на страницу.</li>
      ...
      <h3 class="ref-subsection-title">Языковая модель: локальная или облачная</h3>
      <p class="ref-subsection-sub">Backend выбирается одной env-переменной <code class="fmt-code">LLM_BACKEND</code>. Промпт и пост-обработка JSON-ответа одинаковы для обоих — это даёт честное сравнение «как если бы был только локальный» vs «как если бы был только облачный».</p>

      <div class="ref-block">
        <v-table density="compact" class="ref-table">
          <thead>
            <tr>
              <th style="width:24%">Backend</th>
              <th style="width:22%">Модель</th>
              <th style="width:18%">Среднее время</th>
              <th>Когда использовать</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><strong>mlx</strong> (локально)</td>
              <td><code class="fmt-code">Qwen3.5-9B-MLX-4bit</code></td>
              <td>~12.5 с</td>
              <td>Приватность данных, оффлайн-работа, отсутствие квоты на внешние API. Запускается нативно на хосте через <code class="fmt-code">mlx_lm.server</code>.</td>
            </tr>
            <tr>
              <td><strong>gigachat</strong> · Lite</td>
              <td><code class="fmt-code">GigaChat-2</code></td>
              <td>~2.5 с</td>
              <td>High-volume обработка, где скорость важнее идеального качества. Самый быстрый вариант.</td>
            </tr>
            <tr>
              <td><strong>gigachat</strong> · Pro</td>
              <td><code class="fmt-code">GigaChat-2-Pro</code></td>
              <td>~6.8 с</td>
              <td>Чемпион по качеству на сложных многоконтрагентных документах. Рекомендуется по умолчанию для облачного режима.</td>
            </tr>
            <tr>
              <td><strong>gigachat</strong> · Max</td>
              <td><code class="fmt-code">GigaChat-2-Max</code></td>
              <td>~6.9 с</td>
              <td>Заточен под длинные документы (до 200 страниц). На коротких бизнес-формах не превосходит Pro.</td>
            </tr>
          </tbody>
        </v-table>
      </div>
      ...
      <p class="ref-note">
        <v-icon size="14" class="mr-1">mdi-shield-lock-outline</v-icon>
        Локальный путь не требует интернета и не отправляет содержимое документов наружу. Облачный — быстрее в 2–5×, но требует ключа GigaChat и сетевого доступа к Sber API. Подробное сравнение — в <code class="fmt-code">docs/llm-comparison.md</code>.
      </p>
    </section>
